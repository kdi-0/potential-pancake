<script lang="ts">
    ...
    function disableButton(val: number) {
...
<div>
    ...
    <input disabled={check } type=number id="intN" size="1" bind:value={N} min=1 max=4 maxlength="1">
    <h3> Matrix size: {N}x{N} </h3>
    <button disabled={disableButton(N)} on:click={() => {check = true;}}>
